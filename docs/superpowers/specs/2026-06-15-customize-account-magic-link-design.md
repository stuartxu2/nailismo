...
- **Email provider: Fastmail** (JMAP API), called via `fetch` (no new npm dependency). Sender: `hello@nailismo.com`. Key: `FASTMAIL_API_TOKEN` (a Fastmail JMAP bearer token, `fmu…`). *(Originally specced as Resend; the supplied token turned out to be a Fastmail JMAP token, so the sender was implemented against Fastmail's JMAP API: session → discover Sent mailbox + identity → `Email/set` + `EmailSubmission/set`.)*
...
### 2. `lib/customize/email.ts` — Fastmail JMAP send
- `sendDesignsReady({email, jobs, loginUrl})` / `sendMagicLink(email, loginUrl)` — GET the JMAP session (`https://api.fastmail.com/jmap/session`) with `Authorization: Bearer ${FASTMAIL_API_TOKEN}`, discover the Sent mailbox + `hello@nailismo.com` identity, then `Email/set` (create) + `EmailSubmission/set` (send) in one request.
...
- `CUSTOMIZE_AUTH_SECRET` and `FASTMAIL_API_TOKEN` live in `.env.local` / Vercel env; never logged.
...
- `email.test.ts` — template contains the magic link, all 3 thumbnails, the credit line; Fastmail JMAP flow (session → discover → Email/set + EmailSubmission/set, mocked fetch); send failure is swallowed.
...
- Fastmail JMAP token (mail + submission scope) with `hello@nailismo.com` as a sending identity. `FASTMAIL_API_TOKEN` — done (+ pushed to Vercel prod).
